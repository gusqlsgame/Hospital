...
<%
	...
	Vector<HospitalDataBean> vlist = null;//getBoardList()메소드의 리턴 타입을 vector<boradBean>으로 선언
	...
	totalRecord = HDB.getTotalCount(keyField, keyWord);
...
<script type="text/javascript">
	function list() {
		document.listFrm.action = "main.jsp?pagefile=./hosBoard/list";
...
<%=pagePerBlock%>
	* (value - 1) + 1;
	...
	function read(idx) {
		...
		document.readFrm.pagefile.value = "./hosBoard/read";
...
<body>
	...
	<table>
		...
		<%
			vlist = HDB.getHDBList(keyField, keyWord, start, end);
		...
		<%
				...
				for (int i = 0; i < numPerPage; i++) {
					if (i == listSize)
						break;
					HospitalDataBean bean = vlist.get(i);
					int idx = bean.getHos_code();
					String name = bean.getHos_name();
 ...
 %> <a href="javascript:read('<%=idx%>')"><%=title%></a>
	...
	%>
	<input type="button" onClick="loginOk()" value="글쓰기">
	<%
		} else {
	%><input type="button"
		onClick="javascript:location.href='main.jsp?pagefile=./hosBoard/post'"
		value="글쓰기">
	<%
		}
	%>
 ...
 %></a></li>
	...
	<form name="searchFrm" method="post"
		action="main.jsp?pagefile=./hosBoard/list">
	...
	<form name="readFrm" method="get">
		<input type="hidden" name="pagefile" value="./hosBoard/list"> <input
